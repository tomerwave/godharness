...
crates.io requires a path dependency to also carry a version, and Cargo cannot inherit the
workspace version into that requirement, so it is written by hand in godharness-cli's Cargo.toml
and checked here too — a release that cannot build is worse than one that was never tagged.
"""
