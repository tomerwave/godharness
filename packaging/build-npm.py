...
binary they can run and nothing else. The front door is `@godharness/cli`: it carries no binary,
...
SCOPE = "@godharness"
REPOSITORY = "https://github.com/tomerwave/godharness"
DESCRIPTION = "Agent-context and documentation-governance framework."
...
def common(version: str) -> dict[str, object]:
    return {
        ...
        "keywords": ["agents", "documentation", "context", "developer-tools"],
    }
...
def platform_package(out: Path, version: str, target: str, binary: Path) -> str:
    ...
    executable = "godharness.exe" if system == "win32" else "godharness"

    directory.mkdir(parents=True)
    shutil.copy2(binary, directory / executable)
    (directory / executable).chmod(0o755)
    (directory / "README.md").write_text(
        f"# {name}\n\nThe godharness binary for {system} {architecture}. "
        f"Install [godharness]({REPOSITORY}#readme) instead of this package.\n",
        encoding="utf-8",
    )
    write(
...
def shim_package(out: Path, version: str, names: list[str]) -> None:
    ...
    shutil.copy2(SHIM, directory / "bin" / "godharness.js")
    (directory / "bin" / "godharness.js").chmod(0o755)
    ...
    write(
        directory / "package.json",
        {
            "name": f"{SCOPE}/cli",
            **common(version),
            "bin": {"godharness": "bin/godharness.js"},
            "files": ["bin/godharness.js", "README.md", "LICENSE"],
...
def binary_for(binaries: Path, target: str) -> Path:
    executable = "godharness.exe" if target.endswith("windows-msvc") else "godharness"
    candidates = (
        binaries / f"binary-{target}" / executable,
        binaries / target / executable,
        binaries / f"godharness-{target}" / executable,
    )
