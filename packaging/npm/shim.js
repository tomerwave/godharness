...
const name = `@godharness/cli-${process.platform}-${process.arch}`;

let binary;

try {
  binary = path.join(path.dirname(require.resolve(`${name}/package.json`)), `godharness${suffix}`);
} catch {
  console.error(
    `godharness: no prebuilt binary for ${process.platform}-${process.arch}.`,
    `\nInstall ${name} directly, or build from source with \`cargo install godharness-cli\`.`
  );
  process.exit(1);
}
...
if (finished.error) {
  console.error(`godharness: could not run ${binary}: ${finished.error.message}`);
  process.exit(1);
}
