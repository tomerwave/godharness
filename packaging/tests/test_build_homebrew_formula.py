...
class BuildHomebrewFormulaTest(unittest.TestCase):
    def test_renders_both_macos_architectures(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = Path(directory) / "Formula" / "godharness.rb"
            assets.mkdir()
            ...
            self.assertEqual(output.read_text(encoding="utf-8"), self.expected_formula())
    ...
    def test_refuses_a_checksum_for_a_different_archive(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            assets.mkdir()
            self.write_checksum(assets, "aarch64-apple-darwin", "a" * 64)
            (assets / "godharness-x86_64-apple-darwin.tar.gz.sha256").write_text(
                f"{'b' * 64}  godharness-aarch64-apple-darwin.tar.gz\n", encoding="utf-8"
            )
    ...
    def run_generator(self, assets: Path, output: Path) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            ["python3", str(GENERATOR), "0.7.0", "--assets", str(assets), "--out", str(output)],
            check=False,
            capture_output=True,
            text=True,
        )
    ...
    def write_checksum(self, assets: Path, target: str, checksum: str) -> None:
        archive = f"godharness-{target}.tar.gz"
        (assets / f"{archive}.sha256").write_text(f"{checksum}  {archive}\n", encoding="utf-8")

    def expected_formula(self) -> str:
        return """class Godharness < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/tomerwave/godharness/releases/download/v0.7.0/godharness-aarch64-apple-darwin.tar.gz"
    ...
    if Hardware::CPU.intel?
      url "https://github.com/tomerwave/godharness/releases/download/v0.7.0/godharness-x86_64-apple-darwin.tar.gz"
